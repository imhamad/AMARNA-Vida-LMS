...
class UserConnectionManager {
  ...
  private function addDeclineLink(array &$links, int $uid, array $class): void {
    ...
    if ($invitation->isAccepted()) {
      ...
      // Add the email link if the invitation is already accepted.
      $this->addMailLink($links, $uid);
    }
    else {
  ...
  /**
   * Add the "Email" action to the list of existing connection links.
   *
   * @param array $links
   *   The list of user connection links.
   * @param int $uid
   *   The ID of user to return the "Email" link for.
   */
  private function addMailLink(array &$links, int $uid): void {
    if (!$this->userStorage instanceof UserStorageInterface) {
      return;
    }
    ...
    // Email should be visible only for not-private profiles or for users with
    // specific permiissions.
    $is_private = $user->hasField('field_private_profile') && (bool) $user->get('field_private_profile')->getString();
    if (!$is_private || $this->userAccessManager->canViewPrivateProfile($user)) {
      $links['mail'] = Url::fromUri('mailto:' . (string) $user->getEmail())->toString();
    }
  }
